...
def valida_cpf(event=None):    
    x=cpf_e_fr1.get().replace('.','').replace('-', '')[:11]
    y=''
    ...
    for i in range(len(x)):
        if x[i] in '0123456789':
            if i in [2,5]:
                ...
                y+=x[i]
    cpf_e_fr1.delete(0, 'end')
    cpf_e_fr1.insert(0, y)
...
#Validação de tel
def valida_tel(event=None):    
    x=tel_e_fr1.get().replace('(', '').replace(')', "").replace('-','')[:11]
    y=''
    if event.keysym.lower() == "backspace": return
    for i in range(len(x)):
        if not x [i] in '0123456789': continue 
        if i in [0]:
         y+= '(' + x [i]      
            ...
            y+=x[i]
    tel_e_fr1.delete(0, 'end')
    tel_e_fr1.insert(0, y)
...
name_e_fr1 = Entry(fr1, fg='blue', font='Georgia 10',textvariable=var)
